...
### 北極星（運営者の根本思想・2026-06-07明文化）
**「不動産業者と一般の人との“情報格差”をなくす」**こと。
業者だけが知る相場・売りやすさ・資産性・取引の裏側（実成約価格／売出価格との差、囲い込み、両手仲介、査定額の盛り等）を、千葉市民が自分で確認できる状態にする。
判断軸：新機能を作るたび「これは業者しか知らなかった情報を、住民が自力で知れるようにするか？」を自問する。誤情報・誇張・営業トークの混入は北極星に反するため厳禁。
...
| 施策 | 状態 | 詳細 |
|------|------|------|
| **A 実成約価格フレーミング** | ✅ 完成（2026-06-07） | 全6区マップ＋35エリアページで「実際に売れた価格（国交省・成約ベース）」と明示。SUUMO等の“売出価格”との差を可視化＝業者との情報格差を埋める中核 |
| **B 売りやすさ診断（点判定）** | ✅ 完成（2026-06-07） | 全6区マップに「🔍売りやすさ診断」。任意地点タップ→用途地域(A29)＋立地適正化(A50)をpoint-in-polygonで判定→売りやすさ・資産性スコアと根拠を表示 |
| **C 本音インサイト** | ✅ 完成（2026-06-07） | 35エリアページに資産性(立地適正化)/住環境(用途地域)/流動性(取引件数)/価格トレンドを全て出典付きで表示。客観事実のみ（営業トーク・推測なし） |
| **住民の声の仕組み** | ✅ 完成（2026-06-07） | qa_questions.source='user'でユーザー投稿を区別予定（列追加SQL未実行・コードは*選択で前方互換）。各エリア/マップから投稿CTA。全6区qaに「TERRA作成の代表的事例（実在の個人投稿ではない）」明示で誠実性担保 |
| **GA4イベント計測** | ✅ 完成（2026-06-07） | G-PRTFHJSJY3へ diag_open/diag_result/town_view/trend_view/satei_click/qa_post_submit を送信。どの機能が使われたかをデータで判断可能に。Playwrightで発火確認済み |
...
| **エリア個別ページ（35本）** | ✅ 完成（2026-06-07） | 他5区主要町丁目に area-*.html を量産（計35本・C本音インサイト/FAQ JSON-LD/SNSシェア付き）。検索長尾を獲得 |
| **SNSシェア/OGP** | ✅ 完成（2026-06-07） | エリアページにLINE/X/FB共有ボタン。OGP画像 ogp.svg を新URLで配信 |
...
### 次の実装優先順位（2026-06-07 更新）

**現状認識**: 技術プラットフォームはCLAUDE.mdの施策①〜⑤＋A/B/C＋計測まで概ね実装完了。
ボトルネックは「実装」ではなく ①運営者しか実行できない2件のSQL ②実ユーザー＝データ蓄積の不在 にある。
データが溜まらなければ Data Moat もフィードバックループも回らないため、ここが最重要。

| 優先度 | タスク | 担当 | 理由 |
|--------|--------|------|------|
| 1 | **運営者しか実行できないSQL 2件** | 運営者 | (a) `scripts/add-qa-source.sql`＝qa_questionsにsource列追加（住民投稿バッジが点く／コードは*選択で前方互換済み）。(b) GRANT残り（notify_subscribers/area_buyer_count/card_usage_logのanon集計）。Supabase SQL Editorで実行するだけ |
| 2 | **集客の初動（最重要）** | 運営者 | 実ユーザーほぼ0＝蓄積データ0。GBP公開・SNS/LINEシェア・団地ポスティング(QR)で最初の訪問を作る。GA4イベント(計測済)で効果測定→使われた機能を伸ばす |
| 3 | **業者との情報格差を“言語化”するコンテンツ** | Claude | 本サービスの北極星＝情報格差解消。「業者が言わない本当のこと」を各ガイド/マップに明文化（実成約vs売出の差・両手仲介・囲い込み・査定額の盛り・専任媒介の罠等を中立解説）。Data Moat兼SEO資産 |
| 4 | **フィードバックループの可視化** | Claude | 「あなたの入力が活きた」瞬間を作る。例: 売りやすさ診断の累計回数・町別の注目度（town_view集計）・買い手人数のリアル表示をユーザーに還元 |
| 5 | **notify_subscribers 他区配信** | Claude | 登録者が増えたら send-inage-notify を ward 対応に汎用化し各区配信 |
